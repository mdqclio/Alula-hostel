          ...
          ${lista.map(h => `
            <div style="background:var(--surface);border:1px solid rgba(220,38,38,0.3);border-radius:var(--radius2);padding:16px 20px;display:flex;align-items:center;gap:16px;">
              <div style="flex:1;">
                ...
                <div style="font-size:12px;color:var(--text3);margin-top:4px;">${escapeHtml(h.motivo || 'Sin detalle')}</div>
              </div>
              ${h.nivel ? `<div><span class="badge red">${escapeHtml(h.nivel)}</span></div>` : ''}
            </div>`).join('')}
          </div>`}
