...
- **Borrar al titular** (Huéspedes → borrar) queda bloqueado mientras sea
  titular de algún grupo que no esté `cancelada`. El aviso siempre empieza con
  "Es titular del grupo X —" y sigue según el estado:
  - `confirmada`: "…cancelá el grupo primero."
  - `checkin` / `checkout`: "…los huéspedes con historial grupal no se pueden
    eliminar." Se hizo así a propósito, para no perder el vínculo entre el
    grupo y su titular.
